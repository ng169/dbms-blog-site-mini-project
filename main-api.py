...
def comment():
    ...
    if request.method == "POST":
        content = request.form.get("content")
        cur.execute("INSERT INTO comment(content,blog_id_comment,uid_comment) VALUES (%s,%s, %s)",
                    (content, blog_id, user_id))
    elif request.method == "PUT":
        content = request.form.get("content")
        cur.execute("UPDATE comment SET content = %s WHERE blog_id_comment = %s AND uid_comment = %s",
                    (content, blog_id, user_id))
    elif request.method == "DELETE":
